...
export async function GET() {
  try {
    ...
    const { data: rows, error } = await supabase
      .from('brick_breaker_high_scores')
      .select('userId, highScore, updatedAt, users(name, active)')
      .order('highScore', { ascending: false })
      .limit(50)

    if (error) {
      console.error('brick_breaker_high_scores GET:', error)
    ...
    console.error('GET /api/games/brick-breaker/scores:', e)
    return NextResponse.json(
      { error: 'Failed to load scores' },
      { status: 500 }
    )
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (fetchErr) {
      console.error('brick_breaker_high_scores fetch:', fetchErr)
      return NextResponse.json(
    ...
    const { error: upsertErr } = await supabase.from('brick_breaker_high_scores').upsert(
      {
        userId: user.id,
        highScore: next,
        updatedAt: now,
    ...
    if (upsertErr) {
      console.error('brick_breaker_high_scores upsert:', upsertErr)
      return NextResponse.json(
        { error: 'Failed to save score' },
        { status: 500 }
      )
    ...
    console.error('POST /api/games/brick-breaker/scores:', e)
    return NextResponse.json(
      { error: 'Failed to save score' },
      { status: 500 }
    )

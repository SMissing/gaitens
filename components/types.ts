...
export interface BrickBreakerLayout {
  ...
  brickGap: number
  /**
   * Playfield ceiling (ratio of canvas height). The ball bounces off this line;
   * align the React HUD so its bottom meets this position.
   */
  topPadding: number
  /** Gap in pixels between the ceiling line and the first row of bricks */
  brickTopGap: number
...
export interface BrickBreakerProps {
  ...
  /**
   * Server-backed personal best (e.g. from API). Merged with local high score;
   * use with `config.storage.persistHighScore: false` for a single source of truth.
   */
  remoteHighScore?: number
  /**
   * `contain` — letterbox to 4:3 inside the play area (default).
